<?php
  ...
  if (isset($_GET["id"]) && $_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    while ($row = mysqli_fetch_array($query)) {
      ...
      <select name="lev[]" id="lev" disabled class="border shadow text-md w-full h-full p-3 border-l text-black rounded-none mb-3">
        <option class="text-xl" value="<?=$row["level"]?>"><?=$row["level"]?></option>
      ...
      <select name="number[]" id="sem" disabled class="border shadow text-md w-full h-full p-3    border-l text-black rounded-none mb-3">
